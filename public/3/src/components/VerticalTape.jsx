/**
 * Vertical Tape Component
 * Similar to ObjectTape but oriented vertically on the right side of screen
 * with semicircles pointing inward (left)
 */

import { useEffect, useRef, useState } from 'react';
import styles from './VerticalTape.module.css';

const VerticalTape = ({ cards, onCardEdit, onCardsReorder }) => {
  ...
  useEffect(() => {
    ...
    // Draw tape line (vertical) on the right side
    const tapeX = width - 150;
    ctx.strokeStyle = '#333';
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(tapeX, 0);
    ...
    // Draw cards (semicircles pointing left from the tape)
    visibleCards.forEach((card, idx) => {
      ...
      // Draw semicircle pointing left (perpendicular to tape, base on tape)
    ...
    // Draw focus indicator (triangle pointing right at the top card)
