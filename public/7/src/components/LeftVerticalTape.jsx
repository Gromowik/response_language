/**
 * Left Vertical Tape Component (Left Vertical - Person 1)
...
const LeftVerticalTape = ({ cards, otherCards, onCardEdit, onCardsReorder }) => {
  ...
  // Helper function to find pair for a card (Person 1 card -> Person 2 card)
  ...
  useEffect(() => {
    ...
    visibleCards.forEach((card, idx) => {
      ...
      // Draw semicircle pointing left (like Right Vertical left tape)
      ...
      // Draw text (to the left of the tape)
      ctx.fillStyle = '#000';
      ctx.font = '12px Arial';
    ...
    // Draw focus indicator (triangle pointing to the left at the top card)
  ...
  useEffect(() => {
    ...
    visibleCards.forEach((card, idx) => {
      ...
      // Draw semicircle pointing left (objects on the left side of the line, like Right Vertical left tape)
      ctx.fillStyle = card.color;
      ctx.beginPath();
      ctx.arc(x, y + CARD_WIDTH / 2, CARD_WIDTH / 2, Math.PI / 2, 3 * Math.PI / 2);
      ctx.fill();
      ...
      // Object is on the left side of the line, vector goes right (outward)
  ...
  const handleSpecialFocusClick = (e) => {
    // Check if click is on special focus circle
    if (specialFocusCardId) {
      ...
      if (specialCard && specialCard._specialFocusXAbsolute !== undefined) {
        ...
        if (dist < specialCard._specialFocusRadius + 5) {
          e.stopPropagation(); // Stop propagation only if clicking on special focus
          if (e.detail === 2) {
    ...
    // If click is not on special focus, allow it to pass through to underlying elements
    // Don't stop propagation - let left canvas handle the click
    return false;
  };
