...
// Base cards for Person 1 (only generated and internal reflections, no external reflections)
const basePerson1Cards = [
  {
    id: '1',
    name: 'Generated One',
...
// Base cards for Person 2 (only generated and internal reflections, no external reflections)
const basePerson2Cards = [
  {
    id: 'p2-1',
...
// Helper function to create external reflection
const createExternalReflectionCard = (sourceCard, reflectionId, createdAt) => {
  const externalReflectionColors = ['#2ECC71', '#52BE80', '#27AE60', '#1ABC9C', '#16A085'];
  const color = externalReflectionColors[Math.floor(Math.random() * externalReflectionColors.length)];
  ...
  return {
    ...
    createdAt: createdAt,
    focusedAt: createdAt,
    reflectionOf: sourceCard.id,
    pairName: sourceCard.name // Name of the paired object on the other person's tape
  };
};

// Create default cards for Person 1 with external reflections from Person 2
export const createDefaultCards = () => {
  // Clone base cards to avoid mutating originals
  const person1Cards = basePerson1Cards.map(card => ({ ...card }));
  const baseTime = Date.now();
  
  // Add external reflections from Person 2's generated/internal cards
  basePerson2Cards.forEach((person2Card, index) => {
    if (person2Card.type === 'generated' || person2Card.type === 'internalReflection') {
      const reflection = createExternalReflectionCard(
        person2Card, 
        `reflection-p2-${person2Card.id}`, 
        baseTime - (basePerson2Cards.length - index) * 1000
      );
      person1Cards.push(reflection);
    }
  });
  
  // Add personId and focusedAt to Person 1 cards
  return person1Cards.map(card => ({
    ...card,
    personId: 1,
    focusedAt: card.focusedAt || card.createdAt
  }));
};

// Create default cards for Person 2 with external reflections from Person 1
export const createDefaultCardsPerson2 = () => {
  // Clone base cards to avoid mutating originals
  const person2Cards = basePerson2Cards.map(card => ({ ...card }));
  const baseTime = Date.now();
  
  // Add external reflections from Person 1's generated/internal cards
  // and link them back to source cards
  basePerson1Cards.forEach((person1Card, index) => {
    if (person1Card.type === 'generated' || person1Card.type === 'internalReflection') {
      const reflection = createExternalReflectionCard(
        person1Card, 
        `reflection-p1-${person1Card.id}`, 
        baseTime - (basePerson1Cards.length - index) * 1000
      );
      person2Cards.push(reflection);
    }
  });
  
  // Add personId and focusedAt to Person 2 cards
  return person2Cards.map(card => ({
    ...card,
    personId: 2,
    focusedAt: card.focusedAt || card.createdAt
  }));
